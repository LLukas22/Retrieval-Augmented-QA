...
def main(fin_path,fout_path):
    ...
    n_parts = 1
    while True:
        if os.path.exists(f"{fin_path}.{n_parts}"):
            n_parts += 1
        else:
            break
    ...
    with open(fout_path, "wb") as fout:
        ...
        for part_id in range(n_parts):
            ...
            fin_path_part = fin_path
            if part_id > 0:
                fin_path_part += f".{part_id}"
            with open(fin_path_part, "rb") as fin:
                read_tokens(fin, read_hparams(fin))
                copy_tensors(fin, fout, part_id, n_parts)

    print(f"Done. Output file: {fout_path}\n")
